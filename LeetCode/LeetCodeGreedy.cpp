...
/// <summary>
/// Leet Code 3683. Earliest Time to Finish One Task
///
/// Easy
///
...
/// 
/// Constraints:
/// 1. 1 <= tasks.length <= 100
/// 2. tasks[i] = [si, ti]
/// 3. 1 <= si, ti <= 100
/// </summary>
int LeetCodeGreedy::earliestTime(vector<vector<int>>& tasks)
{
    int result = INT_MAX;
    for (size_t i = 0; i < tasks.size(); i++)
    {
...
/// </summary>
long long LeetCodeGreedy::minEnergy(int n, int brightness, vector<vector<int>>& intervals)
{
    sort(intervals.begin(), intervals.end());
    int total_time = 0;
    int start = -1, end = -1;
