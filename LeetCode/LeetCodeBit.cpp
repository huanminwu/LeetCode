...
/// </summary>
int LeetCodeBit::uniqueXorTripletsI(vector<int>& nums)
{
...
/// 2. 1 <= nums[i] <= 1500
/// </summary>
int LeetCodeBit::uniqueXorTripletsII(vector<int>& nums)
{
    unordered_set<int> xor_set, result;
    for (size_t i = 0; i < nums.size(); i++)
    {
        for (size_t j = 0; j < nums.size(); j++)
        {
            xor_set.insert(nums[i] ^ nums[j]);
        }
    }
    for (size_t i = 0; i < nums.size(); i++)
    {
...
/// 4. 1 <= m <= n
/// </summary>
int LeetCodeBit::maximumAND(vector<int>& nums, int k, int m)
{
    int n = nums.size();
    int result = 0;
    for (int i = 30; i >= 0; i--)
    {
        ...
        for (int j = 0; j < n; j++)
        {
            ...
            {
                int bit = 1 << p;
                if (((bit_mask & bit) != 0) && ((nums[j] & bit) == 0))
                {
                    cost[j] = bit_mask - nums[j];
                    break;
                }
                else if (((bit_mask & bit) == 0) && ((nums[j] & bit) != 0))
                {
                    bit_mask |= bit;
                }
            }
        ...
        for (int j = 0; j < m; j++) total_cost += cost[j];
        if (total_cost <= (long long)k)
        {
            result = target;
        }
    }
...
/// 3. 0 <= k < 2^15
/// </summary>
int LeetCodeBit::maxXor(vector<int>& nums, int k)
{
    ...
    TrieBitNode trie(15);
    trie.increase(0, 1);
    vector<int> nums_xor(n);
    for (right = 0; right < n; right++)
    {
        heap.insert({ nums[right], right });
        nums_xor[right] = nums[right];
        if (right > 0)
        {
            nums_xor[right] ^= nums_xor[right - 1];
        }
        trie.increase(nums_xor[right], 1);
        while (heap.rbegin()->first - heap.begin()->first > k)
        {

            if (left == -1) trie.decrease(0, 1);
            else trie.decrease(nums_xor[left], 1);
            left++;
            heap.erase({ nums[left], left });
        }
        result = max(result, (int)trie.maxXorFind(nums_xor[right]));
    }
    return result;
}
...
/// </summary>
string LeetCodeBit::maximumXor(string s, string t)
{
    vector<int> count(2);
    for (size_t i = 0; i < t.size(); i++)
    {
...
/// 4. 1 <= grid[i][j] <= 10^5
/// </summary>
int LeetCodeBit::minimumOR(vector<vector<int>>& grid)
{
    int result = 0, mask = 0;
    for (int i = 20; i >= 0; i--)
    {
        ...
        for (size_t j = 0; j < grid.size(); j++)
        {
            ...
            {
                if (((grid[j][k] & mask) | result) != result)
                {
                    continue;
                }
                if ((grid[j][k] & bit) == 0)
