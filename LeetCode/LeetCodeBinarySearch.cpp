...
/// 5. 1 <= li <= 10^9
/// </summary>
double LeetCodeBinarySearch::separateSquares(vector<vector<int>>& squares)
{
    double min_val = INT_MAX, max_val = INT_MIN;
    for (size_t i = 0; i < squares.size(); i++)
    {
        min_val = min(min_val, (double)squares[i][1]);
        max_val = max(max_val, (double)squares[i][1] + (double)squares[i][2]);
    }
    while (max_val - min_val >= (double)1e-5)
    {
        double line = (min_val + max_val) / 2;
        double above = 0.0, below = 0.0;

        for (size_t i = 0; i < squares.size(); i++)
        {
            double bottom = (double)squares[i][1];
            double top = (double)squares[i][1] + (double)squares[i][2];
            if (bottom >= line)
            {
                above += (double)squares[i][2] * (double)squares[i][2];
            }
            else if (top <= line)
            {
                below += (double)squares[i][2] * (double)squares[i][2];
            }
            else
            {
                above += (top - line) * (double)squares[i][2];
                below += (line - bottom) * (double)squares[i][2];
            }
        }
        if (above > below)
        {
    ...
    double result = (min_val + max_val) / 2;
    return result;
}
...
/// 3. 1 <= m <= 10^9
/// </summary>
long long LeetCodeBinarySearch::maxScore(vector<int>& points, int m)
{
    ...
    {
        long long middle = (first + last) / 2;
        long long ops = 0, transit = 0;
        vector<double> dp(n);
        for (int i = 0; i < n; i++)
        {
            if (dp[i] < middle)
            {
                long long op = (long long)((middle - dp[i] + points[i] - 1) / points[i]);
                dp[i] += (double)points[i] * op;
                if (i < n - 1)
                {
                    dp[i + 1] += (double)points[i + 1] * (op - 1);
                }
                ops += op + op - 1;
                ops += transit;
                transit = 0;
...
/// <summary>
/// Leet Code 3506. Find Time Required to Eliminate Bacterial Strains
///
/// Hard
///
...
/// 
/// Constraints:
/// 1. 2 <= timeReq.length <= 10^5
/// 2. 1 <= timeReq[i] <= 10^9
/// 3. 1 <= splitTime <= 10^9
/// </summary>
long long LeetCodeBinarySearch::minEliminationTime(vector<int>& timeReq, int splitTime)
{
    ...
    while (first <= last)
    {
        long long middle = first + (last - first) / 2;
        int count = 1;
        long long leadingTime = 0;
        for (int i = n - 1; i >= 0; i--)
        {
            while (timeReq[i] + leadingTime + splitTime <= middle && count < i + 1 && count > 0)
            {
                leadingTime += splitTime;
                count *= 2;
            }
            if (count == 0 || timeReq[i] + leadingTime > middle)
            {
                first = middle + 1;
                break;
            }
...
/// <summary>
/// Leet Code 3520. Minimum Threshold for Inversion Pairs Count
///
/// Medium
///
/// You are given an array of integers nums and an integer k.
/// An inversion pair with a threshold x is defined as a pair of 
...
/// There are less than k inversion pairs if we choose any integer less 
/// than 8 as threshold.
/// 
/// Constraints:
...
/// 3. 1 <= k <= 10^9
/// </summary>
int LeetCodeBinarySearch::minThreshold(vector<int>& nums, int k)
{
    set<int> sorted_set;
    for (int x : nums) sorted_set.insert(x);
    vector<int> sorted_arr;
    for (int x : sorted_set) sorted_arr.push_back(x);
    int first = 0, last = sorted_arr.back() - sorted_arr[0];
    int result = -1;
    while (first <= last)
    {
        BinaryIndexTree bit(sorted_arr.size() + 1);
        int middle = first + (last - first) / 2;
        long long count = 0;
        for (size_t i = 0; i < nums.size(); i++)
        {
            int index1 = lower_bound(sorted_arr.begin(), sorted_arr.end(), nums[i]) - sorted_arr.begin() + 1;
            int index2 = upper_bound(sorted_arr.begin(), sorted_arr.end(), nums[i] + middle) - sorted_arr.begin();
            count += bit.sum(index2) - bit.sum(index1);
            bit.add(index1, 1);
        }
        if (count >= k)
        {
...
/// <summary>
/// Leet Code 3605. Minimum Stability Factor of Array
///
/// Hard
///
...
/// Since maxC = 1, one optimal strategy is to change nums[1] to 7, resulting 
/// in nums = [3, 7, 10].
...
/// 3. 0 <= maxC <= n
/// </summary>
int LeetCodeBinarySearch::minStable(vector<int>& nums, int maxC)
{
    int n = nums.size();
    int first = 0;
    int last = n;
...
/// <summary>
/// Leet Code 3710. Maximum Partition Factor
///
/// Hard
///
...
/// 3. -10^8 <= xi, yi <= 10^8
/// </summary>
int LeetCodeBinarySearch::maxPartitionFactor(vector<vector<int>>& points)
{
    int first = 0, last = (int)1e9;
    int result = 0;
    int n = points.size();
    if (n <= 2) return 0;
        ...
        for (int i = 0; i < n && valid; i++)
        {
            if (group[i] == -1)
            {
                ...
                while (valid && !queue.empty())
                {
                    ...
                    int group_id = group[node];
                    queue.pop();
                    for (size_t j = 0; j < distances[node].size(); j++)
                    {
                        if (distances[node][j].first < middle)
                        {
                            int next = distances[node][j].second;
                            if (group[next] == -1)
                            {
                                group[next] = 1 - group_id;
                                queue.push(next);
                            }
                            else if (group[next] == group_id)
                            {
                                valid = false;
                                break;
                            }
...
/// <summary>
/// Leet Code #3971. Maximum Total Value
///
/// Hard
///
...
/// decay[i] represents how much the value decreases after each selection 
/// of index i.
/// You may select any index multiple times.The total number of selections 
/// across all indices must not exceed m.
///
/// If you select index i for the tth time, where t is 1 - indexed, the 
/// value gained is value[i] - decay[i] * (t - 1).
///
...
/// 
/// Constraints:
/// 1. 1 <= value.length == decay.length <= 10^5
/// 2. 1 <= value[i], decay[i] <= 10^9
/// 3. 1 <= m <= 10^9
/// </summary>
int LeetCodeBinarySearch::maxTotalValue(vector<int>& value, vector<int>& decay, int m)
{
    int max_val = 0;
    for (size_t i = 0; i < value.size(); i++)
    {
        max_val = max(value[i], max_val);
    }
    long long first = 0, last = max_val;
    long long threshold_base = 0;
    ...
    while (first <= last)
    {
        ...
        for (size_t i = 0; i < value.size(); i++)
        {
            if ((long long)value[i] < middle) continue;
            count += ((long long)value[i] - middle) / decay[i] + 1;
        }
        if (count < (long long)m)
        {
            last = middle - 1;
        }
        else
        {
            threshold_base = middle;
    ...
    for (size_t i = 0; i < value.size(); i++)
    {
        if ((long long)value[i] < threshold_base) continue;
        long long count = ((long long)value[i] - threshold_base) / decay[i];
        long long base = (long long)value[i] - (long long)decay[i] * count;
        result += ((long long)value[i] + base) * (((long long)value[i] - base) / decay[i] + 1) / 2;
        if (threshold_count > m && base == threshold_base)
        {
            threshold_count--; 
            result -= base;
        }
