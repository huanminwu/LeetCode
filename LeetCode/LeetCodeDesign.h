...
/// <summary>
/// Leet Code 3709. Design Exam Scores Tracker
///
/// Medium
///
...
/// startTime and endTime (inclusive). If there are no recorded exams 
/// taken by Alice within the specified time interval, return 0.
...
/// examTracker.totalScore(1, 1); 
/// // Between time 1 and time 1, Alice took 1 exam at time 1, scoring 98. 
/// // The total score is 98.
/// examTracker.record(5, 99); 
/// // Alice takes a new exam at time 5, scoring 99.
/// examTracker.totalScore(1, 3); 
/// // Between time 1 and time 3, Alice took 1 exam at time 1, scoring 98. 
/// // The total score is 98.
/// examTracker.totalScore(1, 5); 
/// // Between time 1 and time 5, Alice took 2 exams at time 1 and 5, 
/// // scoring 98 and 99. The total score is 98 + 99 = 197.
/// examTracker.totalScore(3, 4); 
/// // Alice did not take any exam between time 3 and time 4. 
/// // Therefore, the answer is 0.
/// examTracker.totalScore(2, 5); 
/// // Between time 2 and time 5, Alice took 1 exam at time 5, 
/// // scoring 99. The total score is 99.
///
/// Constraints:
/// 1. 1 <= time <= 10^9
/// 2. 1 <= score <= 10^9
/// 3. 1 <= startTime <= endTime <= t, where t is the value of time from 
///    the most recent call of record().
/// 4. Calls of record() will be made with strictly increasing time.
/// 5. After ExamTracker(), the first function call will always be record().
/// 6. At most 105 calls will be made in total to record() and totalScore().
/// </summary>
class ExamTracker 
{
private:
    vector<pair<int, long long>> m_records;
public:
    ExamTracker() 
    {
        m_records.push_back({ 0, 0 });
    }

    void record(int time, int score) 
    {
        m_records.push_back({ time, score + m_records.back().second });
    }
    long long totalScore(int startTime, int endTime) 
    {
        int first = lower_bound(m_records.begin(), m_records.end(), 
            make_pair(startTime, 0LL)) - m_records.begin() - 1;
        int second = upper_bound(m_records.begin(), m_records.end(), 
            make_pair(endTime, LLONG_MAX)) - m_records.begin(); 
        return m_records[second - 1].second - m_records[first].second;
    }
};

/// <summary>
/// Leet Code 3815. Design Auction System
///
/// Medium
///
...
/// // return 2 as User 2 has the highest bid
/// auctionSystem.updateBid(1, 7, 8); 
/// // User 1 updates bid to 8 on item 7
/// auctionSystem.getHighestBidder(7); 
/// // return 1 as User 1 now has the highest bid
/// auctionSystem.removeBid(2, 7); 
/// // Remove User 2's bid on item 7
/// auctionSystem.getHighestBidder(7); 
/// // return 1 as User 1 is the current highest bidder
/// auctionSystem.getHighestBidder(3); 
/// // return -1 as no bids exist for item 3
///
/// Constraints:
/// 1. 1 <= userId, itemId <= 5 * 10^4
/// 2. 1 <= bidAmount, newAmount <= 10^9
/// 3. At most 5 * 104 total calls to addBid, updateBid, removeBid, and 
/// getHighestBidder.
/// The input is generated such that for updateBid and removeBid, the 
/// bid from the given userId for the given itemId will be valid.
/// </summary>
class AuctionSystem 
{
private:
    unordered_map<int, unordered_map<int, int>> m_userBids;
    unordered_map<int, set<pair<int, int>>> m_Itembids;
public:
    AuctionSystem() 
    {
    ...
    {
        if (m_userBids[userId].count(itemId) > 0)
        {
            m_Itembids[itemId].erase(make_pair(m_userBids[userId][itemId], userId));
        }
        m_userBids[userId][itemId] = bidAmount;
        m_Itembids[itemId].insert(make_pair(bidAmount, userId));
    }
    ...
    {
        m_Itembids[itemId].erase(make_pair(m_userBids[userId][itemId], userId));
        m_userBids[userId].erase(itemId);
    }

    int getHighestBidder(int itemId) 
    {
        if (m_Itembids[itemId].empty() || m_Itembids[itemId].empty())
        {
            return - 1;
        }
        else
        {
            return m_Itembids[itemId].rbegin()->second;
        }
    }
};
...
/// <summary>
/// Leet Code 3822. Design Order Management System
///
/// Medium
///
...
/// vector<int> getOrdersAtPrice(string orderType, int price) : Returns 
/// the orderIds of all active orders that match the given orderType and 
/// price.If no such orders exist, return an empty list.
...
/// // A buy order with ID 1 is added at price 1.
/// orderManagementSystem.addOrder(2, "buy", 1); 
/// // A buy order with ID 2 is added at price 1.
/// orderManagementSystem.addOrder(3, "sell", 2); 
/// // A sell order with ID 3 is added at price 2.
/// orderManagementSystem.getOrdersAtPrice("buy", 1); 
/// // Both buy orders (IDs 1 and 2) are active at price 1, so the result is [2, 1].
/// orderManagementSystem.modifyOrder(1, 3); 
/// // Order 1 is updated: its price becomes 3.
/// orderManagementSystem.modifyOrder(2, 1); 
/// // Order 2 is updated, but its price remains 1.
/// orderManagementSystem.getOrdersAtPrice("buy", 1); 
/// // Only order 2 is still an active buy order at price 1, so the result is [2].
/// orderManagementSystem.cancelOrder(3); 
/// // The sell order with ID 3 is canceled and removed from active orders.
/// orderManagementSystem.cancelOrder(2); 
/// // The buy order with ID 2 is canceled and removed from active orders.
/// orderManagementSystem.getOrdersAtPrice("buy", 1); 
/// // There are no active buy orders left at price 1, so the result is [].
///
/// Constraints:
/// 1. 1 <= orderId <= 2000
/// 2. orderId is unique across all orders.
/// 3. orderType is either "buy" or "sell".
/// 4. 1 <= price <= 10^9
...
{
private:
    unordered_map<int, pair<string, int>> m_orders;
    unordered_map<string, unordered_map<int, set<int>>> m_orderBook;
public:
    ...
    {
        m_orderBook[orderType][price].insert(orderId);
        m_orders[orderId] = make_pair(orderType, price);
    }

    void modifyOrder(int orderId, int newPrice) 
    {
        m_orderBook[m_orders[orderId].first][m_orders[orderId].second].erase(orderId);
        m_orders[orderId].second = newPrice;
        m_orderBook[m_orders[orderId].first][newPrice].insert(orderId);
    }
    ...
    {
        vector<int> result;
        for (auto order : m_orderBook[orderType][price])
        {
