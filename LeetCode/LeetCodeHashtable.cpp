...
/// <summary>
/// Leet Code 3527. Find the Most Common Response
///
/// Medium
///
...
///
/// Example 1:
/// Input: responses = [["good","ok","good","ok"],["ok","bad","good","ok",
/// "ok"],["good"],["bad"]]
///
/// Output: "good"
///
...
/// </summary>
string LeetCodeHashtable::findCommonResponse(vector<vector<string>>& responses)
{
    ...
    {
        if ((itr.second > count) || ((itr.second == count) && (itr.first < result)))
        {
            count = itr.second;
            result = itr.first;
        }
...
/// 3. 1 <= k <= nums.length
/// </summary>
bool LeetCodeHashtable::partitionArray(vector<int>& nums, int k)
{
    ...
    for (auto itr : num_count)
    {
        if (itr.second > group) return false;
    }
    return true;
}
...
/// 1. 1 <= n <= 2^31 - 1
/// </summary>
int LeetCodeHashtable::getLeastFrequentDigit(int n)
{
...
/// </summary>
vector<int> LeetCodeHashtable::recoverOrder(vector<int>& order, vector<int>& friends)
{
    unordered_set<int> friend_list;
    for (int i : friends)
    {
        friend_list.insert(i);
    }
    vector<int> result;
    for (size_t i = 0; i < order.size(); i++)
    {
        if (friend_list.count(order[i]) > 0)
        {
...
/// <summary>
/// Leet Code 3731. Find Missing Elements
///
/// Easy
///
...
/// should be [1,2,3,4,5]. The missing integers are 2, 3, and 4.
/// 
/// Constraints:
/// 1. 2 <= nums.length <= 100
/// 2. 1 <= nums[i] <= 100
/// </summary>
vector<int> LeetCodeHashtable::findMissingElements(vector<int>& nums)
{
    unordered_set<int> num_set;
    int min_num = INT_MAX;
    int max_num = INT_MIN;
...
/// <summary>
/// Leet Code 3761. Minimum Absolute Distance Between Mirror Pairs
///
/// Medium
/// 
/// You are given an integer array nums.
...
/// Return the minimum absolute distance between the indices of any 
/// mirror pair.The absolute distance between indices i and j is 
/// abs(i - j).
///
...
/// 2. 1 <= nums[i] <= 10^9
/// </summary>
int LeetCodeHashtable::minMirrorPairDistance(vector<int>& nums)
{
    unordered_map<int, int> reverse_map;
    int result = INT_MAX;
    for (size_t i = 0; i < nums.size(); i++)
    {
        if (reverse_map.count(nums[i]) > 0)
        {
            result = min(result, (int)i - reverse_map[nums[i]]);
        }
        string str_num = to_string(nums[i]);
        reverse(str_num.begin(), str_num.end());
        reverse_map[atoi(str_num.c_str())] = i;
    }
...
/// <summary>
/// Leet Code 3810. Minimum Operations to Reach Target Array 
/// 
/// Medium
///
...
/// maximal if it cannot be extended to the left or right while keeping 
/// all values equal to x)
/// For each such segment[l, r], update simultaneously :
/// nums[l] = target[l], nums[l + 1] = target[l + 1], ..., 
/// nums[r] = target[r]
/// Return the minimum number of operations required to make nums equal 
/// to target.
///
/// Example 1:
...
/// Output : 1
/// Explanation :
/// Choose x = 4 : maximal segments[0, 0] and [2, 2] updated(nums[2] 
/// stays 4)->nums becomes[5, 1, 4]
...
/// Output : 2
/// Explanation :
/// Choose x = 7 : maximal segments[0, 0] and [2, 2] updated->nums 
/// becomes[5, 3, 9]
/// Choose x = 3 : maximal segment[1, 1] updated->nums becomes[5, 5, 9]
/// Thus, 2 operations are required to convert nums to target.
/// 
/// Constraints :
/// 1. 1 <= n == nums.length == target.length <= 10^5
/// 2. 1 <= nums[i], target[i] <= 10^5
/// </summary>
int LeetCodeHashtable::minOperations(vector<int>& nums, vector<int>& target)
{
    unordered_map<int, int> num_count;
    for (size_t i = 0; i < nums.size(); i++)
    {
...
/// 2. 0 <= nums[i] <= 10^5
/// </summary>
int LeetCodeHashtable::mostFrequentEven(vector<int>& nums)
{
