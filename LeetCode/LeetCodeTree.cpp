...
/// </summary>
int LeetCodeTree::treeQueries(int parent, int node, vector<vector<pair<int, int>>>& neighbors,
    vector<int>& weights, BinaryIndexTree& bit, vector<int>& tin, vector<int>& tout, int index)
{
    tin[node] = index;
    bit.add(index, weights[node]);
    index++;
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        if (neighbors[node][i].first == parent)
        {
            continue;
        }
        weights[neighbors[node][i].first] = neighbors[node][i].second;
        index = treeQueries(node, neighbors[node][i].first, neighbors, weights, bit, tin, tout, index);
    }
    tout[node] = index;
    bit.add(index, -weights[node]);
    return index;
...
/// 13. 1 <= w' <= 10^4
/// </summary>
vector<int> LeetCodeTree::treeQueries(int n, vector<vector<int>>& edges, vector<vector<int>>& queries)
{
    ...
    vector<int> tin(n + 1), tout(n + 1), weights(n + 1);
    treeQueries(0, 1, neighbors, weights, bit, tin, tout, 1);
    vector<int> result;
    for (size_t i = 0; i < queries.size(); i++)
    {
        if (queries[i][0] == 2)
        ...
        else
        {
            ...
            if (tin[parent] > tin[child])
            {
                swap(parent, child);
            }
            int delta = queries[i][3] - weights[child];
            weights[child] = queries[i][3];
            bit.add(tin[child], delta);
            bit.add(tout[child], -delta);
        }
    }
    return result;
}
...
/// </summary>
unordered_map<int, int> LeetCodeTree::goodSubtreeSum_dfs(vector<vector<int>>& neighbors, vector<int>& vals, int node, int parent, int& sum)
{
    int M = 1000000007;
    unordered_map<int, int> result_set;
    int val = vals[node];
    int mask = 0;
    while (val > 0)
    {
    ...
    if (mask > 0)
    {
        result_set[mask] = vals[node];
    }
    for (auto child : neighbors[node])
    {
        if (child == parent) continue;
        unordered_map<int, int> temp_set = result_set;
        unordered_map<int, int> sub_set = goodSubtreeSum_dfs(neighbors, vals, child, node, sum);
        for (auto& [mask1, val1] : sub_set) 
        {
            temp_set[mask1] = max(temp_set[mask1], val1);
            for (auto& [mask2, val2] : result_set) 
            {
                if ((mask1 & mask2) == 0) 
                {
                    temp_set[mask1 | mask2] = max(temp_set[mask1 | mask2], val1 + val2);
                }
            }
        }
        result_set = move(temp_set);
    }

    int max_val = 0;
    for (auto& [mask, val] : result_set) 
    {
...
///    valid tree.
/// </summary>
int LeetCodeTree::goodSubtreeSum(vector<int>& vals, vector<int>& par)
{
    vector<vector<int>> neighbors(par.size());
    for (size_t i = 0; i < par.size(); i++)
    {
        if (par[i] == -1) continue;
        neighbors[par[i]].push_back(i);
    }
    int result = 0;
...
/// <summary>
/// Leet Code 3593. Minimum Increments to Equalize Leaf Paths
/// </summary>
long long LeetCodeTree::minIncrease_dfs(int node, int parent, vector<vector<int>>& neighbors, vector<int>& cost, vector<long long>& max_path)
{
    ...
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        int child = neighbors[node][i];
        if (child == parent) continue;
        max_value = max(max_value, minIncrease_dfs(child, node, neighbors, cost, max_path));
    }
    max_path[node] = max_value + cost[node];
...
/// Leet Code 3593. Minimum Increments to Equalize Leaf Paths
/// </summary>
int LeetCodeTree::minIncrease_inc(int node, int parent, vector<vector<int>>& neighbors, vector<int>& cost, vector<long long>& max_path, long long inc)
{
    int count = 0;
    inc += cost[node];
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        int child = neighbors[node][i];
        if (child == parent) continue;
        if (max_path[child] + inc < max_path[0])
        {
            count++;
        }
        count += minIncrease_inc(child, node, neighbors, cost, max_path, max_path[0] - max_path[child]);
    }
    return count;
}
...
/// n nodes numbered from 0 to n - 1. This is represented by a 2D array edges 
/// of length n - 1, where edges[i] = [ui, vi] indicates an edge from node ui 
/// to vi .
///
...
/// 7. The input is generated such that edges represents a valid tree.
/// </summary>
int LeetCodeTree::minIncrease(int n, vector<vector<int>>& edges, vector<int>& cost)
{
    vector<vector<int>> neighbors(n);
    for (size_t i = 0; i < edges.size(); i++)
    {
    ...
    vector<long long> max_path(n);
    minIncrease_dfs(0, -1, neighbors, cost, max_path);
    int result = minIncrease_inc(0, -1, neighbors, cost, max_path, 0);
    return result;
}

/// <summary>
/// Leet Code 3772. Maximum Subgraph Score in a Tree
/// </summary>
int LeetCodeTree::maxSubgraphScoreDFSInPath(int parent, int node, vector<vector<int>>& neighbors,
    vector<int>& good, vector<int>& inPathScores)
{
    inPathScores[node] = (good[node] == 1) ? 1 : -1;
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        if (neighbors[node][i] == parent) continue;
        int score = maxSubgraphScoreDFSInPath(node, neighbors[node][i], neighbors, good, inPathScores);
        inPathScores[node] += max(0, score);
    }
    return inPathScores[node];
}

/// <summary>
/// Leet Code 3772. Maximum Subgraph Score in a Tree
/// </summary>
int LeetCodeTree::maxSubgraphScoreDFSOutPath(int parent, int node, vector<vector<int>>& neighbors,
    vector<int>& inPathScores, vector<int>& outPathScores)
{
    if (parent == -1)
    {
        outPathScores[node] = 0;
    }
    else
    {
        outPathScores[node] = outPathScores[parent] + (inPathScores[parent] - max(0,inPathScores[node]));
        outPathScores[node] = max(0, outPathScores[node]);
    }
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        if (neighbors[node][i] == parent) continue;
        int score = maxSubgraphScoreDFSOutPath(node, neighbors[node][i], neighbors, inPathScores, outPathScores);
    }
    return outPathScores[node];
}
...
///
/// You are also given an integer array good of length n, where 
/// good[i] is 1 if the ith node is good, and 0 if it is bad.
///
/// Define the score of a subgraph as the number of good nodes minus 
/// the number of bad nodes in that subgraph.
///
/// For each node i, find the maximum possible score among all 
/// connected subgraphs that contain node i.
///
...
/// original graph.
///
/// A connected subgraph is a subgraph in which every pair of its 
/// vertices is reachable from one another using only its edges.
///
/// Example 1:
/// Tree Example 1
...
/// For each node, the best connected subgraph containing it is the whole 
/// tree, which has 2 good nodes and 1 bad node, resulting in a score of 1.
/// Other connected subgraphs containing a node may have the same score.
/// 
/// Example 2 :
/// Tree Example 2
/// Input : n = 5, edges = [[1, 0], [1, 2], [1, 3], [3, 4]], 
/// good = [0, 1, 0, 1, 1]
/// Output : [2, 3, 2, 3, 3]
/// Explanation :
/// Node 0 : The best connected subgraph consists of nodes 0, 1, 3, 4, 
/// which has 3 good nodes and 1 bad node, resulting in a score 
/// of 3 - 1 = 2.
/// Nodes 1, 3, and 4 : The best connected subgraph consists of 
/// nodes 1, 3, 4, which has 3 good nodes, resulting in a score of 3.
/// Node 2 : The best connected subgraph consists of nodes 1, 2, 3, 4, 
/// which has 3 good nodes and 1 bad node, resulting in a score 
/// of 3 - 1 = 2.
///
/// Example 3 :
...
/// Output : [-1, -1]
/// Explanation :
/// For each node, including the other node only adds another bad node, 
/// so the best score for both nodes is - 1.
/// 
/// Constraints:
/// 1. 2 <= n <= 10^5
/// 2. edges.length == n - 1
...
/// 7. The input is generated such that edges represents a valid tree.
/// </summary>
vector<int> LeetCodeTree::maxSubgraphScore(int n, vector<vector<int>>& edges, vector<int>& good)
{
    vector<vector<int>> neighbors(n), treeNodes(n, { 0, 0 });
    for (size_t i = 0; i < edges.size(); i++)
    {
        neighbors[edges[i][0]].push_back(edges[i][1]);
        neighbors[edges[i][1]].push_back(edges[i][0]);
    ...
    vector<int> result;
    vector<int> inPathScores(n), outPathScores(n);
    maxSubgraphScoreDFSInPath(-1, 0, neighbors, good, inPathScores);
    maxSubgraphScoreDFSOutPath(-1, 0, neighbors, inPathScores, outPathScores);
    for (int i = 0; i < n; i++)
    {
        result.push_back(inPathScores[i] + outPathScores[i]);
    }
    return result;
}
...
/// </summary>
void LeetCodeTree::interactionCosts(int parent, int node, vector<vector<int>>& neighbors,
    vector<vector<int>>& node_groups, vector<int>& group)
{
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        ...
        interactionCosts(node, child, neighbors, node_groups, group);
        for (size_t j = 0; j < node_groups[node].size(); j++)
        {
            node_groups[node][j] += node_groups[child][j];
        }
    }
    node_groups[node][group[node]-1]++;
}

/// <summary>
/// Leet Code 3786. Total Sum of Interaction Cost in Tree Groups
/// </summary>
long long LeetCodeTree::interactionCosts(int parent, int node, vector<vector<int>>& neighbors,
    vector<vector<int>>& node_groups)
{
    ...
    for (size_t i = 0; i < neighbors[node].size(); i++)
    {
        ...
        result += interactionCosts(node, child, neighbors, node_groups);
        for (size_t j = 0; j < node_groups[child].size(); j++)
        {
            result += (long long)node_groups[child][j] * (node_groups[0][j] - node_groups[child][j]);
        }
    }
    return result;
}
...
/// 7. The input is generated such that edges represents a valid tree.
/// </summary>
long long LeetCodeTree::interactionCosts(int n, vector<vector<int>>& edges, vector<int>& group)
{
    vector<vector<int>> neighbors(n);
    for (size_t i = 0; i < edges.size(); i++)
    {
    ...
    int group_size = 0;
    for (size_t i = 0; i < group.size(); i++) group_size = max(group_size, group[i]);
    vector<vector<int>> node_groups(n, vector<int>(group_size));
    interactionCosts(-1, 0, neighbors, node_groups, group);
    long long result = interactionCosts(-1, 0, neighbors, node_groups);
    return result;
}
...
/// </summary>
vector<int> LeetCodeTree::findSpecialNodes_BFS(int start_node, vector<vector<int>>& neighbors)
{
...
/// 5. The input is generated such that edges represents a valid tree.
/// </summary>
string LeetCodeTree::findSpecialNodes(int n, vector<vector<int>>& edges)
{
    vector<vector<int>> neighbors(n);
    for (size_t i = 0; i < edges.size(); i++)
    {
    ...
    vector<int> special_nodes = findSpecialNodes_BFS(0, neighbors);
    for (size_t i = 0; i < special_nodes.size(); i++)
    {
        int start_node = special_nodes[i];
        result[start_node] = '1';
        vector<int> next_nodes = findSpecialNodes_BFS(start_node, neighbors);
        for (size_t j = 0; j < next_nodes.size(); j++)
        {
            result[next_nodes[j]] = '1';
        }
    }
    return result;
}
...
/// <summary>
/// Leet Code 3812. Minimum Edge Toggles on a Tree
///
/// Hard
/// 
/// You are given an undirected tree with n nodes, numbered from 0 to 
...
/// desired color.
///
/// In one operation, you may pick an edge with index i and toggle both 
/// of its endpoints.That is, if the edge is[u, v], then the colors of 
/// nodes u and v each flip from '0' to '1' or from '1' to '0'.
///
...
/// [1, 6]], start = "0011000", target = "0010001"
/// Output : [1, 2, 5]
/// Explanation :
...
/// After these operations, the resulting string becomes "0010001", 
...
/// Constraints:
/// 1. 2 <= n == start.length == target.length <= 10^5
/// 2. edges.length == n - 1
/// 3. edges[i] = [ai, bi]
/// 4. 0 <= ai, bi < n
...
/// 7. The input is generated such that edges represents a valid tree.
/// </summary>
vector<int> LeetCodeTree::minimumFlips(int n, vector<vector<int>>& edges, string start, string target)
{
    vector<int> degree(n);
    vector<vector<pair<int, int>>> neighbors(n);
    for (size_t i = 0; i < edges.size(); i++)
    ...
    while (!queue.empty())
    {
        int node = queue.front();
        queue.pop();
        if (degree[node] == 1) degree[node]--;
        bool toggle = false;
        if (start[node] != target[node]) toggle = true;
        for (size_t i = 0; i < neighbors[node].size(); i++)
        {
            ...
            if (toggle == true)
            {
                start[next] = '0' + 1 - (start[next] - '0');
                toggle = false;
                result.push_back(neighbors[node][i].second);
            }
        }
        if (toggle == true)
        {
...
/// <summary>
/// Leet Code 3820. Pythagorean Distance Nodes in a Tree
/// </summary>
vector<int> LeetCodeTree::specialNodes(int root, vector<vector<int>>& neighbors)
{
    vector<int> distances(neighbors.size(), -1);
    queue<int> queue;
    queue.push(root);
    int distance = 0;
    while (!queue.empty())
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            for (size_t j = 0; j < neighbors[node].size(); j++)
            {
                if (distances[neighbors[node][j]] == -1)
                {
                    queue.push(neighbors[node][j]);
                }
            }
...
/// Medium
///
/// You are given an integer n and an undirected tree with n nodes 
/// numbered from 0 to n - 1. The tree is represented by a 2D array edges 
/// of length n - 1, where edges[i] = [ui, vi] indicates an undirected 
...
///
/// The distance between two nodes in a tree is the number of edges on 
/// the unique path between them.
/// 
/// Example 1:
/// Input: n = 4, edges = [[0, 1], [0, 2], [0, 3]], x = 1, y = 2, z = 3
/// Output : 3
/// Explanation :
/// For each node, we compute its distances to nodes x = 1, y = 2, 
/// and z = 3.
///
/// Node 0 has distances 1, 1, and 1. After sorting, the distances 
/// are 1, 1, and 1, which do not satisfy the Pythagorean condition.
/// Node 1 has distances 0, 2, and 2. After sorting, the distances 
/// are 0, 2, and 2. Since 0^2 + 2^2 = 2^2, node 1 is special.
/// Node 2 has distances 2, 0, and 2. After sorting, the distances 
/// are 0, 2, and 2. Since 0^2 + 2^2 = 2^2, node 2 is special.
/// Node 3 has distances 2, 2, and 0. After sorting, the distances 
/// are 0, 2, and 2. This also satisfies the Pythagorean condition.
...
/// Output : 0
/// Explanation :
/// For each node, we compute its distances to nodes x = 0, y = 3, 
/// and z = 2.
/// 
/// Node 0 has distances 0, 3, and 2. After sorting, the distances 
/// are 0, 2, and 3, which do not satisfy the Pythagorean condition.
/// Node 1 has distances 1, 2, and 1. After sorting, the distances 
/// are 1, 1, and 2, which do not satisfy the Pythagorean condition.
/// Node 2 has distances 2, 1, and 0. After sorting, the distances 
/// are 0, 1, and 2, which do not satisfy the Pythagorean condition.
/// Node 3 has distances 3, 0, and 1. After sorting, the distances 
/// are 0, 1, and 3, which do not satisfy the Pythagorean condition.
...
/// Output : 1
/// Explanation :
/// For each node, we compute its distances to nodes x = 1, y = 3, 
/// and z = 0.
///
/// Node 0 has distances 1, 2, and 0. After sorting, the distances 
/// are 0, 1, and 2, which do not satisfy the Pythagorean condition.
/// Node 1 has distances 0, 1, and 1. After sorting, the distances 
/// are 0, 1, and 1. Since 0^2 + 12 = 1^2, node 1 is special.
/// Node 2 has distances 1, 2, and 2. After sorting, the distances 
/// are 1, 2, and 2, which do not satisfy the Pythagorean condition.
/// Node 3 has distances 1, 0, and 2. After sorting, the distances 
/// are 0, 1, and 2, which do not satisfy the Pythagorean condition.
...
/// 3. edges[i] = [ui, vi]
/// 4. 0 <= ui, vi, x, y, z <= n - 1
/// 5. x, y, and z are pairwise distinct.
/// 6. The input is generated such that edges represent a valid tree.
/// </summary>
int LeetCodeTree::specialNodes(int n, vector<vector<int>>& edges, int x, int y, int z)
{
    vector<vector<int>> neighbors(n);
    ...
    vector<int> distances_x = specialNodes(x, neighbors);
    vector<int> distances_y = specialNodes(y, neighbors);
    vector<int> distances_z = specialNodes(z, neighbors);
    int result = 0;
    for (int i = 0; i < n; i++)
    {
        distances[i][0] = distances_x[i];
        distances[i][1] = distances_y[i];
        distances[i][2] = distances_z[i];
        sort(distances[i].begin(), distances[i].end());
        if (distances[i][0] * distances[i][0] + distances[i][1] * distances[i][1] ==
            distances[i][2] * distances[i][2])
        {
            result++;
        }
    }
...
/// <summary>
/// Leet Code 3902. Zigzag Level Sum of Binary Tree
///
///  Medium
/// 
/// You are given the root of a binary tree.
/// Traverse the tree level by level using a zigzag pattern :
/// At odd - numbered levels(1 - indexed), traverse nodes from left 
/// to right.
/// At even - numbered levels, traverse nodes from right to left.
/// While traversing a level in the specified direction, process nodes 
/// in order and stop immediately before the first node that violates 
/// the condition :
/// At odd levels : the node does not have a left child.
/// At even levels : the node does not have a right child.
/// Only the nodes processed before this stopping condition contribute 
/// to the level sum.
///
/// Return an integer array ans where ans[i] is the sum of the node 
/// values that are processed at level i + 1.
///
/// Example 1:
...
/// Explanation :
///
/// At level 1, nodes are processed left to right.Node 5 is included, 
/// thus ans[0] = 5.
/// At level 2, nodes are processed right to left.Node 8 is included, 
/// but node 2 lacks a right child, so processing stops, thus ans[1] = 8.
/// At level 3, nodes are processed left to right.The first node 1 lacks 
/// a left child, so no nodes are included, and ans[2] = 0.
/// Thus, ans = [5, 8, 0].
///
/// Example 2:
...
/// Output : [1, 5, 0]
/// Explanation :
/// At level 1, nodes are processed left to right.Node 1 is included, 
/// thus ans[0] = 1.
/// At level 2, nodes are processed right to left.Nodes 3 and 2 are 
/// included since both have right children, thus ans[1] = 3 + 2 = 5.
/// At level 3, nodes are processed left to right.The first node 4 
/// lacks a left child, so no nodes are included, and ans[2] = 0.
/// Thus, ans = [1, 5, 0].
///
/// Constraints:
...
/// -10^5 <= Node.val <= 10^5
/// </summary>
vector<long long> LeetCodeTree::zigzagLevelSum(TreeNode* root)
{
    ...
    while (!queue.empty())
    {
        ...
        for (size_t i = 0; i < size; i++)
        {
            ...
            else
            {
                ...
                if (node->right != nullptr) queue.push_front(node->right);
                if (node->left != nullptr) queue.push_front(node->left);
            }
        }
