...
--  | 3            | 4           | 2016-06-10 |
--
--  Write a query to find the overall acceptance rate of requests rounded 
--  to 2 decimals, which is the number of acceptance divide the number of 
--  requests.
--
...
-----------------------------------------------------------------------
SELECT
    CASE WHEN B.requests = 0 
        THEN CONVERT(NUMERIC(18, 2), 0)
        ELSE CONVERT(NUMERIC(18, 2), CONVERT(NUMERIC(18, 2), A.accepts) / B.requests) END
        AS accept_rate
FROM
(
    SELECT COUNT(*) AS accepts
    FROM
    (
        SELECT
            DISTINCT 
